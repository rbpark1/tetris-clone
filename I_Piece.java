...
/**
 * Class for the I Tetronimo of the game.
 * 
 * @author "Tetris" Group P4
 * @version 5/18/15
 */
public class I_Piece extends Shape {
    
    /**
     * Constructor that accepts four Block() objects to create a I Tetronimo.
     * @param i1 - A Block() object.
     * @param i2 - A Block() object.
     * @param i3 - A Block() object.
     * @param i4 - A Block() object.
     */
    public I_Piece(Block i1, Block i2, Block i3, Block i4){
        parts[0] = i1;
        parts[1] = i2;
        parts[2] = i3;
        parts[3] = i4;
    ...
    public void shift() {
        if (Greenfoot.isKeyDown("left") && canMoveLeft()) {
            ...
            for (Block x : parts) {
             ...
             if(parts[0].getY()==parts[1].getY()&&parts[0].getY()!=0&&parts[0].getY()<=17){
                            parts[0].setLocation(parts[0].getX() + 1, parts[0].getY()-1);
                            parts[2].setLocation(parts[2].getX() - 1, parts[2].getY()+1);
                            parts[3].setLocation(parts[3].getX() -2, parts[3].getY()+2);
            } 
            
              else if (parts[0].getX()==parts[1].getX()&&parts[0].getX()!=0 &&parts[0].getX()!=8&&parts[0].getY()!=0){
                            parts[0].setLocation(parts[0].getX() -1 , parts[0].getY()+1);
                            parts[2].setLocation(parts[2].getX() + 1, parts[2].getY()-1);
                            parts[3].setLocation(parts[3].getX() +2, parts[3].getY()-2);
            }
